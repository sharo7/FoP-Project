...
void costume_editor(const char* initialized_image)
{
    ...
    {
        // Pre-fill the save path so the user can just press Save without typing anything
        state.UI.filename = string(initialized_image);

        // Try to load the image directly onto the canvas for immediate editing.
        // If loading fails (new/blank file) the canvas stays at Default_Canvas_Color.
        if (!Png_Loader_On_Canvas(state.canvas, initialized_image))
        {
            // File doesn't exist or is unreadable — start with a blank canvas
            for (int i=0;i<canvas_height;i++)
                for (int j=0;j<canvas_width;j++)
                    state.canvas.Canvas_Pixels[i][j] = Default_Canvas_Color;
        }
        // Do NOT enter placing mode — the image is already on the canvas, ready to edit
    ...
    // Show the pre-filled save path in the window title so the user knows
    // where their work will go when they press Save.
    if (!state.UI.filename.empty())
    {
        string title = "Sprite Editor  —  saving to: " + state.UI.filename;
